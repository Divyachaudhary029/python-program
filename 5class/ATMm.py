...
def connection():
    a = sqlite3.connect('ATM09.db')
    return a

def createTab(a):
    a.execute('''CREATE TABLE ATM09(ID INTEGER  PRIMARY KEY AUTOINCREMENT, NAME TEXT,PASSWORD INT,AMOUNT INT)''')
...
def Insert(a):
    ...
    for i in range(n):
        ...
        a.execute("INSERT INTO ATM09(NAME,PASSWORD,AMOUNT) VALUES(?,?,?)",(AA1,AA2,AA3))

def retrieve(a):
    cursor = a.cursor()
    id = int(input("Enter Your I'd :"))
    User = int(input("Enter Your PIN :"))
    penny.append(id)
    B1 = "SELECT * FROM ATM09 WHERE ID =?"
    A1 = "SELECT * FROM ATM09 WHERE PASSWORD=?"
    if A1 and B1 :
...
def update(a):
    ...
    hq=penny[0]
    fc=GG[0]
    zef="UPDATE ATM09 SET AMOUNT=? WHERE ID=?"
